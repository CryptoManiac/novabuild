    ...
    case "${CUR_PLATFORM}" in
        i686)
            ...
            if [ ${need_rebuild} -eq 1 ]; then
                ...
                PATH=$PATH:/usr/i686-w64-mingw32/bin ./configure --host=i686-w64-mingw32 --prefix=/usr/i686-w64-mingw32 --disable-sdltest --without-tools --without-tests || exit_error "configure failed"
            ...
            if [ ${need_rebuild} -eq 1 ]; then
                ...
                sh ../dist/configure --host=i686-w64-mingw32 --enable-cxx --enable-mingw || exit_error "configure failed"
...
#                sed -i 's/wingenminiupnpcstrings \$/wine \.\/wingenminiupnpcstrings \$/' Makefile.mingw
            ...
            if [ ${need_rebuild} -eq 1 ]; then
                ...
                echo "using gcc : 4.4 : i686-w64-mingw32-g++ : <rc>i686-w64-mingw32-windres <archiver>i686-w64-mingw32-ar ;" > user-config.jam
            ...
            if [ ${need_rebuild} -eq 1 ]; then
                echo "Building qt..."
                cd ${platform_src_dir}/qt-everywhere-opensource-src-4.8/ || exit_error "Failed to change to qt source dir"
        ...
        x86_64)
            ...
            if [ ${need_rebuild} -eq 1 ]; then
                echo "Building qrencode..."
                cd ${platform_src_dir}/qrencode-3.4.2/ || exit_error "Failed to change to qrencode-3.4.2/ dir"
                PATH=$PATH:/usr/x86_64-w64-mingw32/bin ./configure --host=x86_64-w64-mingw32 --prefix=/usr/x86_64-w64-mingw32 --disable-sdltest --without-tools --without-tests || exit_error "configure failed"
...
#                sed -i 's/wingenminiupnpcstrings \$/wine \.\/wingenminiupnpcstrings \$/' Makefile.mingw
            ...
            if [ ${need_rebuild} -eq 1 ]; then
                ...
                cd ${platform_src_dir}/boost_1_50_0/ || exit_error "Failed to change to boost_1_50_0/ dir"
                ./bootstrap.sh --without-icu || exit_error "bootstrap failed"
                echo "using gcc : 4.4 : x86_64-w64-mingw32-g++ : <rc>x86_64-w64-mingw32-windres <archiver>x86_64-w64-mingw32-ar ;" > user-config.jam
                ./bjam toolset=gcc target-os=windows variant=release threading=multi threadapi=win32 --user-config=user-config.jam -j 2 --without-mpi --without-python -sNO_BZIP2=1 -sNO_ZLIB=1 --layout=tagged stage
            fi
